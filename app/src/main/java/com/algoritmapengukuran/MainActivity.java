...
public class MainActivity extends AppCompatActivity implements ExampleAdapter.OnBtnDetailClickListener {
    ...
    private void parseJSON() {
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, Constant.API_URL, null, response -> {
            try {
                JSONArray jsonArray = response.getJSONArray("algoritmapengurutan");

                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject hit = jsonArray.getJSONObject(i);

                    String logoUrl = hit.getString("gambar");
                    String versi = hit.getString("algoritma");
                    String deskripsi = hit.getString("deskripsi");
                    String website = hit.getString("baca_lebih_lanjut");
